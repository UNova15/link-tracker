...
public class SubscriptionDao {
    ...
    public List<Long> findChatsIdByLinkId(long linkId) {
        return jdbcClient.sql("SELECT chat_id FROM subscriptions WHERE link_id=:linkId")
            .param("linkId", linkId)
            .query(Long.class)
            .list();
    }
    ...
    public List<Long> findLinksIdByChatId(long chatId) {
        return jdbcClient.sql("SELECT link_id FROM subscriptions WHERE chat_id=:chatId")
            .param("chatId", chatId)
            .query(Long.class)
            .list();
    }
    ...
    public boolean exists(long chatId, long linkId) {
        return jdbcClient.sql("SELECT EXISTS(SELECT 1 FROM subscriptions WHERE chat_id=:chatId AND link_id=:linkId)")
            .param("chatId", chatId)
            .param("linkId", linkId)
            .query(Boolean.class)
            .single();
    ...
    public void saveSubscriptionRecord(long chatId, long linkId) {
        jdbcClient.sql("INSERT INTO subscriptions (chat_id,link_id) VALUES (:chatId,:linkId)")
            .param("chatId", chatId)
            .param("linkId", linkId)
            .update();
    }
    ...
    public void saveTagsForSubscription(Subscription subscription) {
        ...
        template.batchUpdate("INSERT INTO tags_subscriptions (chat_id,link_id,tag) VALUES (?,?,?)", new BatchPreparedStatementSetter() {
            @Override
    ...
    public void removeSubscription(long chatId, long linkId) {
        jdbcClient.sql("DELETE FROM subscriptions WHERE chat_id=:chatId AND link_id=:linkId")
            .param("chatId", chatId)
            .param("linkId", linkId)
            .update();
    }
    ...
    public Optional<Subscription> findUsersSubscription(long chatId, long linkId) {
        List<Subscription> subscriptions = jdbcClient.sql("""
                SELECT s.chat_id, s.link_id, t.tag
